...
static void PlayWavFromMemoryRange(BYTE* wavBuf, size_t wavSize, float startSec, float endSec)
{
    ...
    if (!played) {
        ...
        WriteLog("ERROR: DirectSound failed to play PCM (err=%u)", err);
        delete[] pcm;
        return;
    }
...
{
    if (index >= g_afsTable.size()) { WriteLog("PlayVoiceAfs: invalid index %u", index); return false; }
    ...
    if (!f.is_open()) { WriteLog("PlayVoiceAfs: cannot open AFS."); return false; }
    f.seekg(e.offset, std::ios::beg);

    BYTE* wavData = new BYTE[e.size];
    f.read((char*)wavData, e.size);
    if (!f) { WriteLog("PlayVoiceAfs: reading block failed."); delete[] wavData; return false; }
